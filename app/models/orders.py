...
def _enum(python_enum: type[enum.Enum]):
    return SAEnum(python_enum, values_callable=lambda e: [m.value for m in e])
...
class Order(Base):
    ...
    status: Mapped[OrderStatus] = mapped_column(_enum(OrderStatus), default=OrderStatus.NEW)
    ...
    payment_status: Mapped[PaymentStatus | None] = mapped_column(_enum(PaymentStatus), default=PaymentStatus.PENDING)
    ...
    order_type: Mapped[OrderType] = mapped_column(_enum(OrderType), default=OrderType.DINE_IN)
...
class OrderReminder(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    order_id: Mapped[int] = mapped_column(ForeignKey("orders.order_id", ondelete="CASCADE"))
    type: Mapped[ReminderType] = mapped_column(_enum(ReminderType))
    ...
    status: Mapped[ReminderStatus] = mapped_column(_enum(ReminderStatus), default=ReminderStatus.PENDING)
